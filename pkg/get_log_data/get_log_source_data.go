...
func Get_log_source_data_main() {
	...
	for blcokFrom.Uint64() <= blockMax {
		...
		get_log_source_data_loop2(target_contract, blcokFrom, blockTo, wg)
		blcokFrom = blockTo
	}
	wg.Wait()
...
func get_log_source_data_loop(target_contract string, blcokFrom *big.Int, blockTo *big.Int, wg *sync.WaitGroup) {
	...
	bar := progressbar.Default(int64(len(logs)))
...
func get_log_source_data_loop2(target_contract string, blcokFrom *big.Int, blockTo *big.Int, wg *sync.WaitGroup) {
	defer wg.Done()
	...
	if target_contract == "ChainLink" {
		...
		log_csv = "chainlink_eth_usd_gas_info_" + blcokFrom.String() + "_" + blockTo.String() + ".csv"
		log_headers = []string{
			"blockNum", "index", "txHash", "gas", "gasPrice", "gasTipCap", "gasFeeCap", "blockAvgGasTip",
		}
		topic0_target = "0x0559884fd3a460db3073b7fc896cc77986f16e378210ded43186175bf646fc5f"
	}
	...
	for _, vLog := range logs {
		...
		if vLog.Topics[0].Hex() == topic0_target {
			...
			block_num := big.NewInt(int64(vLog.BlockNumber))
			...
			// iter through the block and get the average gas tip cap
			var gas_tip_sum int64 = 0
			var gas_tip_count int64 = 0
			for _, tx := range block.Transactions() {
				gas_tip_sum += tx.GasTipCap().Int64()
				gas_tip_count += 1
			}
			gas_tip_avg := gas_tip_sum / gas_tip_count
			...
			topics[5] = tx.GasTipCap().String()
			topics[6] = tx.GasFeeCap().String()
			topics[7] = strconv.FormatInt(gas_tip_avg, 10)
